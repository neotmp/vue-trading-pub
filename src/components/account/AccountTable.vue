...
const { useAccount, data, useBroker } = store()
...
function accountType(num: number): string | undefined {
  switch (num) {
    case 1:
      return 'Trade'
    case 2:
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
              <tr v-for="account in accountsList" :key="account.id">
                ...
                <td class="px-4 py-4 text-sm whitespace-nowrap space-x-1 flex justify-end">
                  <RouterLink :to="`/accounts/${account.id}/edit`" class="py-4"
                    ><EditIcon
                  /></RouterLink>
                  <div class="py-4"><TrashbinIcon /></div>
                  <RouterLink :to="`/accounts/${account.id}/detail`" class="py-4"
                    ><DocIcon
                  /></RouterLink>
                  <RouterLink :to="`/accounts/${account.id}/analytics`" class="py-4"
                    ><PiechartIcon
                  /></RouterLink>
                  <button
